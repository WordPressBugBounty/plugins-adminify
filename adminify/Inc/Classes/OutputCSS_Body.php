...
class OutputCSS_Body
{
    ...
    public function output_styles() {
		...
		if (!empty($menu_typography['font-size'])) {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-menu-font-size'] =  esc_attr($menu_typography['font-size'] . 'px');
			} else {
		...
		if (!empty($menu_typography['line-height'])) {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-menu-line-height'] =  esc_attr($menu_typography['line-height'] . 'px');
			} else {
		...
		if (!empty($menu_typography['letter-spacing'])) {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-menu-letter-spacing'] =  esc_attr($menu_typography['letter-spacing'] . 'px');
			} else {
		...
		if (!empty($menu_styles['menu_width']['width'])) {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-menu-width'] =  esc_attr($menu_styles['menu_width']['width'] . 'px');
			} else {
		...
		if (!empty($menu_styles['menu_wrapper_padding']['top']) && $menu_styles['menu_wrapper_padding']['top'] !== '') {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-menu-wrapper-padding-top'] =  esc_attr($menu_styles['menu_wrapper_padding']['top']) . esc_attr($menu_styles['menu_wrapper_padding']['unit']);
			} else {
		...
		if (!empty($menu_styles['menu_wrapper_padding']['right']) && $menu_styles['menu_wrapper_padding']['right'] !== '') {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-menu-wrapper-padding-right'] =  esc_attr($menu_styles['menu_wrapper_padding']['right']) . esc_attr($menu_styles['menu_wrapper_padding']['unit']);
			} else {
		...
		if (!empty($menu_styles['menu_wrapper_padding']['bottom']) && $menu_styles['menu_wrapper_padding']['bottom'] !== '') {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-menu-wrapper-padding-bottom'] =  esc_attr($menu_styles['menu_wrapper_padding']['bottom']) . esc_attr($menu_styles['menu_wrapper_padding']['unit']);
			} else {
		...
		if (!empty($menu_styles['menu_wrapper_padding']['left']) && $menu_styles['menu_wrapper_padding']['left'] !== '') {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-menu-wrapper-padding-left'] =  esc_attr($menu_styles['menu_wrapper_padding']['left']) . esc_attr($menu_styles['menu_wrapper_padding']['unit']);
			} else {
		...
		if (!empty($menu_styles['submenu_wrapper_padding']['top']) && $menu_styles['submenu_wrapper_padding']['top'] !== '') {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-submenu-wrapper-padding-top'] = esc_attr($menu_styles['submenu_wrapper_padding']['top']) . esc_attr($menu_styles['submenu_wrapper_padding']['unit']);
			} else {
		...
		if (!empty($menu_styles['submenu_wrapper_padding']['right']) && $menu_styles['submenu_wrapper_padding']['right'] !== '') {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-submenu-wrapper-padding-right'] =  esc_attr($menu_styles['submenu_wrapper_padding']['right']) . esc_attr($menu_styles['submenu_wrapper_padding']['unit']);
			} else {
		...
		if (!empty($menu_styles['submenu_wrapper_padding']['bottom']) && $menu_styles['submenu_wrapper_padding']['bottom'] !== '') {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-submenu-wrapper-padding-bottom'] =  esc_attr($menu_styles['submenu_wrapper_padding']['bottom']) . esc_attr($menu_styles['submenu_wrapper_padding']['unit']);
			} else {
		...
		if (!empty($menu_styles['submenu_wrapper_padding']['left']) && $menu_styles['submenu_wrapper_padding']['left'] !== '') {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-submenu-wrapper-padding-left'] =  esc_attr($menu_styles['submenu_wrapper_padding']['left']) . esc_attr($menu_styles['submenu_wrapper_padding']['unit']);
			} else {
		...
		if (!empty($this->options['menu_layout_settings']['layout_type']) && $this->options['menu_layout_settings']['layout_type'] === 'vertical') {
			if (!empty($menu_styles['menu_vertical_padding'])) {
				if($this->adminify_ui) {
					$pxlbsadminify_css_var['--adminify-menu-vertical-padding'] = esc_attr($menu_styles['menu_vertical_padding']) . 'px';
				} else {
					$default_ui_css .= '#adminmenu { display: flex; flex-direction: column; row-gap: ' . esc_attr($menu_styles['menu_vertical_padding']) .'px;}';
				}
			}
		}
		// Horizontal-menu padding (layout_type === 'horizontal') is a
		// Pro-only feature. Pro/Classes/OutputCSS_Body_Pro.php hooks
		// pxlbsadminify_menu_layout_css_vars to inject the value when Pro
		// is active. Free leaves it alone.
		$pxlbsadminify_css_var = (array) apply_filters( 'pxlbsadminify_menu_layout_css_vars', $pxlbsadminify_css_var, $menu_styles, $this->options, $this->adminify_ui );

		// Submenu Item Padding
        if (!empty($menu_styles['submenu_vertical_space'])) {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-submenu-vertical-padding'] = esc_attr($menu_styles['submenu_vertical_space']) . 'px';
			} else {
		...
		if (!empty($menu_colors['wrap_bg'])) {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-menu-bg'] = esc_attr($menu_colors['wrap_bg']);
			} else {
		...
		if (!empty($menu_colors['hover_bg'])) {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-menu-hover-bg'] = esc_attr($menu_colors['hover_bg']);
			} else {
		...
		if (!empty($menu_colors['text_color'])) {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-menu-text-color'] = esc_attr($menu_colors['text_color']);
			} else {
		...
		if (!empty($menu_colors['text_hover'])) {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-menu-text-hover-color'] = esc_attr($menu_colors['text_hover']);
			} else {
		...
		if (!empty($menu_colors['active_bg'])) {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-menu-active-bg'] = esc_attr($menu_colors['active_bg']);
			} else {
		...
		if (!empty($menu_colors['active_color'])) {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-menu-active-color'] = esc_attr($menu_colors['active_color']);
			} else {
		...
		if (!empty($submenu_colors['wrap_bg'])) {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-submenu-wrapper-bg'] = esc_attr($submenu_colors['wrap_bg']);
			} else {
		...
		if (!empty($submenu_colors['hover_bg'])) {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-submenu-hover-bg'] = esc_attr($submenu_colors['hover_bg']);
			} else {
		...
		if (!empty($submenu_colors['text_color'])) {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-submenu-text-color'] = esc_attr($submenu_colors['text_color']);
			} else {
		...
		if (!empty($submenu_colors['text_hover'])) {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-submenu-text-hover-color'] = esc_attr($submenu_colors['text_hover']);
			} else {
		...
		if (!empty($submenu_colors['active_bg'])) {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-submenu-active-bg'] = esc_attr($submenu_colors['active_bg']);
			} else {
		...
		if (!empty($submenu_colors['active_color'])) {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-submenu-active-color'] = esc_attr($submenu_colors['active_color']);
			} else {
		...
		// 		$pxlbsadminify_output_css .= '.wp_adminify_user-details a { color:' . esc_attr($this->options['user_info_style']['info_text_color']) . ' !important;}';
		// 	}
		// 	if (!empty($this->options['info_text_hover_color'])) {
		// 		$pxlbsadminify_output_css .= '.wp_adminify_user-details a:hover { color:' . esc_attr($this->options['user_info_style']['info_text_hover_color']) . ' !important;}';
		// 	}
		// 	if (!empty($this->options['user_info_style']['info_text_border'])) {
		// 		if (!empty($this->options['user_info_style']['info_text_border']['all'])) {
		// 			$pxlbsadminify_output_css .= '.wp_adminify_user {
		...
		// 		$pxlbsadminify_output_css .= '.wp_adminify_user-actions i { color:' . esc_attr($this->options['user_info_style']['info_icon_color']) . ' !important;}';
		// 	}
		// 	if (!empty($this->options['user_info_style']['info_icon_hover_color'])) {
		// 		$pxlbsadminify_output_css .= '.wp_adminify_user-actions i:hover { color:' . esc_attr($this->options['user_info_style']['info_icon_hover_color']) . ' !important;}';
		// 	}
		// }
		...
		if (!empty($menu_styles['notif_colors']['notif_bg'])) {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-notif-bg-color'] = esc_attr($menu_styles['notif_colors']['notif_bg']);
			} else {
		...
		if (!empty($menu_styles['notif_colors']['notif_color'])) {
			if($this->adminify_ui) {
				$pxlbsadminify_css_var['--adminify-notif-color'] = esc_attr($menu_styles['notif_colors']['notif_color']);
			} else {
        ...
        // return $pxlbsadminify_output_css;

		if($this->adminify_ui){
			return $pxlbsadminify_css_var;
		} else {
			return $pxlbsadminify_css_var = $default_ui_css;
		}
	}

	public function logo_text_styles () {
