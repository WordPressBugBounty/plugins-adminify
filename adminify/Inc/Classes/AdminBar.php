...
use PXLBSAdminify\Inc\Classes\DarkModeConflicts;
...
?>
	...
	{
		return 'PXLBSAdminify\Inc\Classes\Adminify_Admin_Bar';
	}

	public function front_bar()
	{
		add_action('admin_init', [$this, 'add_admin_bar']);
	...
	public function legacy_admin_bar_positon()
	{
		if (!empty($this->options['admin_bar_position']) && $this->options['admin_bar_position'] == 'bottom') {
			...
			echo '<style>' . esc_html(wp_strip_all_tags($pxlbsadminify_thirdparty_css)) . '</style>';
		}
	}
					...
					<div id="wp-adminify-top-adminbar" class="navbar-menu">
						...
						<div class="navbar-end">
							<div class="field is-grouped">
								...
								<?php echo wp_kses_post($this->dark_mode_switcher_dom()) ?>

								<?php if (!empty($this->options['admin_bar_notif'])) { ?>
								...
								<div class="wp-adminify--preview">
									<a class="preview-trigger is-clickable" href="<?php echo esc_url(get_home_url()); ?>" target="_blank">
								...
								<div class="wp-adminify--user--account">
									<button class="user-avatar is-clickable">
										<div class="image is-45x45">
											<?php echo get_avatar($current_user->user_email, 45, '', '', ['class' => 'is-rounded']); ?>
										</div>
										<span class="user-status tag p-0 is-rounded"></span>
									</button>

									<?php if (!empty($this->adminify_ui)) { ?>
										<div class="wp-adminify--user--wrapper">
											<div class="wp-adminify-user-info">
												<h3 class="wp-adminify-user-name">
													<?php echo esc_html($current_user->display_name); ?>
												</h3>
												<span><?php echo wp_kses_post(is_email($current_user->user_email)); ?></span>
												...
												<ul>
													...
													<li>
														<a href="<?php echo esc_url(admin_url('profile.php')); ?>">
															...
															<span><?php esc_html_e('View Profile', 'adminify'); ?></span>
														</a>
													</li>
													<hr>
													<li>
														<a href="<?php echo esc_url(wp_logout_url(home_url())); ?>" class="wp-adminify-logout">
															...
															<span><?php echo esc_html__('Log Out', 'adminify'); ?></span>
														</a>
													</li>
												</ul>
											</div>
	...
	<?php
		...
		echo wp_kses( $output_wp_admin_bar, Utils::kses_allowed_html() );
	}
	...
	public function logo()
	{
		...
		// Light Logo
		// menu_layout_settings.layout_type / .menu_mode were read here
		// previously but the resulting locals were never used
		// downstream. Removed because layout_type === 'horizontal' is
		// a Pro-only feature whose handler now lives in
		// Pro/Classes/OutputCSS_Body_Pro.php.
	...
	?>
		...
		<div class="navbar-brand">
			<a class="navbar-item p-0" href="<?php echo esc_url($homeurl); ?>">
				<?php if ($admin_ui_logo_type == 'image_logo') { ?>
					<!-- Light Logo -->
					<img alt="<?php echo esc_attr(get_bloginfo('name')); ?>" class="logo-light" src="<?php echo esc_url($light_logo); ?>" width="<?php echo esc_attr($light_width); ?>" height="<?php echo esc_attr($light_height); ?>">
					<img alt="<?php echo esc_attr(get_bloginfo('name')); ?>" class="mini-logo-light" src="<?php echo esc_url($light_mini_logo); ?>" width="36" height="36">
					<!-- Dark Logo -->
					<img alt="<?php echo esc_attr(get_bloginfo('name')); ?>" class="logo-dark" src="<?php echo esc_url($dark_logo); ?>" width="<?php echo esc_attr($dark_width); ?>" height="<?php echo esc_attr($dark_height); ?>">
					<img alt="<?php echo esc_attr(get_bloginfo('name')); ?>" class="mini-logo-dark" src="<?php echo esc_url($dark_mini_logo); ?>" width="36" height="36">
				<?php } elseif ($admin_ui_logo_type == 'text_logo') { ?>
			...
			<!-- <div class="navbar-burger" data-target="wp-adminify-top-adminbar">
				<img src="<?php // echo PXLBSADMINIFY_ASSETS_IMAGE . 'icons/toggol.svg';
							?>"  alt="Toggol icon"/>
			</div> -->
